...
class Config:
    ...
    def read_yaml(self, file_path):
        with open(file_path, 'r') as f:
            ...
            self.feature_dims = dic['feature_dims']
            self.n_gpu = dic['n_gpu']
            self.n_threads = dic['n_threads']
            self.n_speaker = dic['n_speaker']
            self.slides = dic['slides']
            self.sample_rate = dic['sample_rate']
            self.fix_len = dic['fix_len']
            self.num_utt_per_class = dic['num_utt_per_class']
            self.num_classes_per_batch = dic['num_classes_per_batch']
            self.batch_nums_per_epoch = dic['batch_nums_per_epoch']
            self.n_fft = dic['n_fft']
            self.n_speaker_test = dic['n_speaker_test']
            self.hop_length = dic['hop_length']